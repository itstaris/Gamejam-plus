...
func move_plataform():
	...
	var duration = move_direction.lenght() / float(move_speed *plataform_center)
	
	var plataform_tween = create_tween().set_loops().set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT).set_delay(duration + WAIT_duration)
